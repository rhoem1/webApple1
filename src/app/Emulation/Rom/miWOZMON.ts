/**
 * WOZMON for Apple I
 *
 * @copyright Steve Wozniak 
 * @website http://github.com/jefftranter/6502/tree/master/asm/a1basic
 */

import Cpu from '../Cpu/Cpu';
import RomIntercept from '../Cpu/RomIntercept';

const monitor = [
	
];

export default function miWOZMON(cpu: Cpu) {
	return new RomIntercept(cpu, 0xFF00, monitor);
}

